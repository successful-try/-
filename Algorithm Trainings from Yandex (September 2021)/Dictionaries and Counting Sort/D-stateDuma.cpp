...
//ѕадает на 15-м тесте, скорее всего, из-за проблемы сравнени€ вещественных чисел//
//Ќа ѕитоне така€ проблема не обнаружена//
int main()
{
    ...
    while(getline(cin, str)){
        ...
        found = str.rfind(" ");                      //Ќаходим первый пробел
        votes = 0;                                   //с конца строки
        for(int i = found+1; i < str.size(); ++i){
            votes = votes*10 + (str[i] - '0');       //ѕереводим строку
        }                                            //в число(считываем голоса)
        party = str.substr(0,found);
        v.push_back(party);                          //«апоминаем последовательность партий
