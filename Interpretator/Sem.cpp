...
#include <map>
#include <vector>
#include "Dolgov_Lex.cpp"

using namespace std;

//template <class T, int max_size >
class Stack_int // для break
{
    int* s;//[max_size];
    int top;
    int max_size;
...
public:
 ...
 //   void reset() { top = 0; };
    int get_size() {return top;} // Текущий размер
...
public:
   ...
   // void reset() { top = 0; };
    ...
    //template <class U>
    void push(Lex i, std::vector<Lex>& answer); // для сборки и обработки ПОЛИЗа
    void push(Lex i);  //!!!!!!!
...
void Stack_int::push(int i)  // обработка ПОЛИЗ
{
...
void Stack_Lex::push(Lex i, std::vector<Lex>& vec)  // создание ПОЛИЗ
{
    if ( !is_full() )
    {
        ...
        {
            map<type_of_lex, int>::iterator it; // указатель на мапу
