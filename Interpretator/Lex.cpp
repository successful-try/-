...
enum type_of_lex //Перечислимый тип  со всеми лексемами
{
    LEX_PROGRAM,
    LEX_INT,
    LEX_REAL,
...
string name_of_lex[] = // Массив строчек для вывода
{
    "LEX_PROGRAM",
    "LEX_INT",
    "LEX_REAL",
...
class IntIdent : public Value_type // Наследование класса Value_type
{
    int value;
...
public:
    ...
    {
        p = new Ident[size=max_size]; // массив из иднетификаторов
        top = 0;
    }
...
int Tabl_Ident::put ( string buf )
{
    for ( int j=0; j<top; j++ )
    {
      if ( buf == p[j].get_name() ) // указатель на класс Ident
          return j;
    }
...
public:
    Lex ( type_of_lex t = LEX_NULL ) // Конструктор для создания объектов
    {
        type_lex = t;
    }

    Lex ( type_of_lex t, int v ) // Конструктор для создания объектов
    {
        type_lex = t; int_value = v;
        is_int = INT;            //Спецификация лексемы
    }

    Lex ( type_of_lex t, double real_v) // Конструктор для создания объектов
    {
    ...
    void set_int_value(int a) {int_value = a;} // jump (номер в ПОЛИЗе)
...
Lex Parser::get_lex ()
{
   //  cout << c << endl;
    CS = H;
    std::map<string,type_of_lex>::iterator it; // указатель на мапу
    ...
    do
    {
        ...
        case H:
            ...
            {
                return Lex(); // конструктор по умолчанию
            }
            else
            {
                return Lex(LEX_ERROR);
