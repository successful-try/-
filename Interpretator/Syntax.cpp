...
class Syntax
{
    ...
    void H();//начало
    void A();// тип данных/EOF
    void B();//Проверка идентификатора
    void Operators();
    void Perem();
    void Const();
    void SC();
    ...
    void Begend(int contbreak = 0);//Составной оператор
...
public:
    Syntax( const char * program ) : scan(program), St(100), st_jump(100) {}; // буд-то вызвали Parser scan(program);
    void analyze();
    void compute();
};
...
bool check_assingment_types(type_of_lex a, type_of_lex b)
{
        ...
        )
    {
        ...
        return false; // если string с int/real
    }
}
                         ...
                         string& lex1_string )
{
    ...
    {
        lex1_int = a.get_int_value();  // значение int
        a_type = LEX_VAL_INT;          // тип возвращаемого значения
    }
...
void Syntax::compute()
{
    ...
    for (int i = 0; i < Poliz.size(); i++)
    {
        ...
        case LEX_JUMP:
        {
            ...
            i++;
            Lex lex = Poliz[i]; // взять следующий элемент
            ...
            {
                a = comp.pop(); //результат вычисления условия для while, for, if
                type_of_lex a_type;
        ...
        case LEX_READ:
        {
            a = comp.pop(); //результат из стека вычислений
            type_of_lex a_type;
            ...
            {
                int aa;
                cin >> aa; // вводим переменную и кладем её в таблицу имён
        ...
        case LEX_ASSIGNMENT:
        {
            ...
            type_of_lex id_type = TID[a.get_int_value()].get_type(); // тип идентификатора


            type_of_lex b_type = b.get_type();  // тип значения
            ...
            else if (b_type == LEX_IDENT)  // a=b
            {
                ...
                Value_type* tmp= TID[b.get_int_value()].get_ptr(); // надо найти значение b
...
void Syntax::A()
{
    ...
    else if(curr_lex.get_type() == LEX_INT) // Тип int
        {
            flag=LEX_VAL_INT;
            gl();
            B();
    ...
    else if(curr_lex.get_type() == LEX_REAL) // Тип real
        {
            flag=LEX_VAL_REAL;
            gl();
            B();
    ...
    else if(curr_lex.get_type() == LEX_STRING) // Тип string
        {
            flag=LEX_VAL_STRING;
            gl();
            B();
...
void Syntax::Const()
{
    ...
    if(curr_lex.get_type() == LEX_VAL_STRING && flag==LEX_VAL_STRING) // Значение string
    {
        Poliz.push_back(curr_lex);
        gl();
        SC();
    }
    else if (curr_lex.get_type() == LEX_VAL_REAL && flag==LEX_VAL_REAL ||  // Значение int/real
    ...
    else if (curr_lex.get_type() == LEX_PLUS) // Унарный плюс
    {
        gl();
    ...
    else if (curr_lex.get_type() == LEX_MINUS) // Унарный минус
    {
        gl();
        if (curr_lex.get_type() == LEX_VAL_REAL && flag==LEX_VAL_REAL ||
             curr_lex.get_type() == LEX_VAL_INT && flag==LEX_VAL_INT)
...
void Syntax::Expr()
{
    bool _exit = false; // Проверка двух подряд переменных
